...
gift = Category.create(category_name: "gift")
ask = Category.create(category_name: "ask")
gratitude = Category.create(category_name: "gratitude")
...
p1.pictures.attach(io: File.open('app/assets/postimages/food_processor.jpeg'), filename: 'food_processor.jpeg', content_type: 'image/jpeg')
p2 = Post.create(text: "Anyone need some storage shelves?", likes: 2, fulfilled: true, category_id: gift.id, user_id: amir.id)
p2.pictures.attach(io: File.open('app/assets/postimages/storage_shelves.jpeg'), filename: 'storage_shelves.jpeg', content_type: 'image/jpeg')
...
p3.pictures.attach(io: File.open('app/assets/postimages/kale.jpeg'), filename: 'kale.jpeg', content_type: 'image/jpeg')
p4 = Post.create(text: "Various dry goods (some opened, others not) that I won't eat, come take your pick!", likes: 2, fulfilled: false, category_id: gift.id, user_id: ibrahim.id)
p4.pictures.attach(io: File.open('app/assets/postimages/flour_bag.jpg'), filename: 'flour_bag.jpg', content_type: 'image/jpeg')
p4.pictures.attach(io: File.open('app/assets/postimages/rolled_oats.jpg'), filename: 'rolled_oats.jpg', content_type: 'image/jpeg')
p4.pictures.attach(io: File.open('app/assets/postimages/brown_sugar.jpeg'), filename: 'brown_sugar.jpeg', content_type: 'image/jpeg')
p4.pictures.attach(io: File.open('app/assets/postimages/dry_beans_pinto.jpg'), filename: 'dry_beans_pinto.jpg', content_type: 'image/jpeg')
...
p5.pictures.attach(io: File.open('app/assets/postimages/lavender.jpg'), filename: 'lavender.jpg', content_type: 'image/jpeg')
...
p6.pictures.attach(io: File.open('app/assets/postimages/throw_pillows.jpeg'), filename: 'throw_pillows.jpeg', content_type: 'image/jpeg')
p6.pictures.attach(io: File.open('app/assets/postimages/desk_lamp.jpeg'), filename: 'desk_lamp.jpeg', content_type: 'image/jpeg')
p6.pictures.attach(io: File.open('app/assets/postimages/soap_dish.jpeg'), filename: 'soap_dish.jpeg', content_type: 'image/jpeg')
p7 = Post.create(text: "We've got some clothes our daughter outgrew - anyone looking for new-to-you baby clothes?", likes: 1, fulfilled: true, category_id: gift.id, user_id: kurt.id)
p7.pictures.attach(io: File.open('app/assets/postimages/baby_clothes.jpeg'), filename: 'baby_clothes.jpeg', content_type: 'image/jpeg')
p8 = Post.create(text: "Does anyone have any basic gardening supplies? Looking for gloves, tools, whatever you have around.", likes: 1, fulfilled: false, category_id: ask.id, user_id: roxanne.id)
p9 = Post.create(text: "Hello all! I want to learn how to knit and would love some knitting needles and yarn. Anyone have any spare sets of needles and/or yarn I could learn with?", likes: 0, fulfilled: true, category_id: ask.id, user_id: jess.id)
p10 = Post.create(text: "My daughter's birthday party is this weekend and we need some crafts. I'm looking for embroidery floss that the kids can make friendship bracelets with. Hoping for bright colors!", likes: 1, fulfilled: false, category_id: ask.id, user_id: farrah.id)
p11 = Post.create(text: "I just bought a secondhand bike that needs a tuneup. Would anyone have the skills and gift of time to show me how to do it myself? I have all the tools and parts!", likes: 0, fulfilled: true, category_id: ask.id, user_id: andrea.id)
p12 = Post.create(text: "I'm repainting a few rooms in my house, maybe some furniture. Does anyone have any extra paint they could pass along? Open to any and all colors!", likes: 2, fulfilled: false, category_id: ask.id, user_id: thyrone.id)
p13 = Post.create(text: "BIG THANKS to Amir for the extra storage shelves! They've helped me really clean up and organize my office space.", likes: 4, fulfilled: false, category_id: gratitude.id, user_id: olenka.id)
p14 = Post.create(text: "Thank you so much to Kurt for the great baby clothes! Our baby girl looks adorable in her new outfits.", likes: 3, fulfilled: false, category_id: gratitude.id, user_id: daniel.id)

puts "Seeding comments..."
c1 = Comment.create(text: "Interested", likes: 0, user_id: stefan.id, post_id: p1.id)
c2 = Comment.create(text: "Would love this!", likes: 0, user_id: anastasia.id, post_id: p1.id)
c3 = Comment.create(text: "I'm interested in any and all of the dry goods.", likes: 0, user_id: bill.id, post_id: p4.id)
c4 = Comment.create(text: "Interested", likes: 0, user_id: parker.id, post_id: p3.id)
c5 = Comment.create(text: "Interested!", likes: 0, user_id: dapo.id, post_id: p3.id)
c6 = Comment.create(text: "Interested in the desk lamp", likes: 0, user_id: victor.id, post_id: p6.id)
c7 = Comment.create(text: "I'd love to stop by for some clippings!", likes: 0, user_id: andrea.id, post_id: p5.id)
c8 = Comment.create(text: "Beautiful! I'm interested", likes: 0, user_id: nadia.id, post_id: p5.id)
c9 = Comment.create(text: "Interested! I can do a lot of things with a lot of kale.", likes: 0, user_id: olivia.id, post_id: p3.id)
c10 = Comment.create(text: "These would go great in my office!", likes: 0, user_id: olenka.id, post_id: p2.id)
c11 = Comment.create(text: "A new parent can never have too many baby clothes - definitely interested", likes: 2, user_id: daniel.id, post_id: p7.id)
c12 = Comment.create(text: "I have some time tomorrow afternoon! I've tuned my own bike for years.", likes: 3, user_id: jasper.id, post_id: p11.id)
c13 = Comment.create(text: "I've got plenty of supplies! PM me to set up a time to stop by and pick out a few things.", likes: 2, user_id: anna.id, post_id: p9.id)
c14 = Comment.create(text: "I've got a bunch of random craft supplies, including embroidery floss. DM to set something up.", likes: 2, user_id: victoria.id, post_id: p10.id)
c15 = Comment.create(text: "We've got some extra small tools you can stop by and grab.", likes: 1, user_id: remi.id, post_id: p8.id)

puts "Seeding subcomments/replies..."
s1 = Subcomment.create(text: "It's yours! PM for pickup.", likes: 1, user_id: amir.id, comment_id: c10.id)
s2 = Subcomment.create(text: "Awesome! Message me for details.", likes: 1, user_id: kurt.id, comment_id: c11.id)
s3 = Subcomment.create(text: "Messaged you! Thanks!", likes: 0, user_id: jess.id, comment_id: c13.id)
